...
extension PhotoMetadataController {
    func tagsFromTextField() -> [String] {
        ...
        let lowercaseTags = commaSeperatedStrings.map { $0.lowercaseString }
        ...
        case (1, 0):
            ...
            locationManager.onLocationFix = { placemark, error in
                if let placemark = placemark {
                    ...
                    guard let name = placemark.name, area = placemark.administrativeArea else { return }
                    
                    self.locationLabel.text = "\(name), \(area)"
                } else {
                    print()
                }
            }
